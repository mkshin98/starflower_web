...
// 2025-05, 별하늘 성도 클릭시 다운로드 하는 함수
function confirmDownload(imageUrl) {
    // 1. 사용자에게 다운로드 여부 확인
    const isConfirmed = confirm("이 이미지를 다운로드하시겠습니까?");

    if (isConfirmed) {
        // 2. 다운로드를 위한 가상 앵커(a) 태그 생성
        const link = document.createElement('a');
        link.href = imageUrl;

        // 3. 파일명 추출 (URL의 마지막 부분) 또는 원하는 파일명 지정
        const fileName = imageUrl.split('/').pop();
        link.download = fileName;

        // 4. body에 붙여서 클릭 이벤트 발생 후 제거
